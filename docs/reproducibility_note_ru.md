...
- strengthened rerun `pilot1k_v2` пока завершён только частично: baseline и оба single-task sweep уже есть, но multitask ещё не завершён по всем target seeds;
...
1. Завершить и агрегировать strengthened rerun под текущим протоколом `epochs=4`, `balanced`, `source-balanced`, `early stopping`.
