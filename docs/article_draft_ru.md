...
Предлагается мультитаск-подход на основе общей трансформерной encoder-архитектуры с двумя классификационными головами: для определения тональности отзыва и для определения его достоверности. В качестве экспериментальной базы рассматриваются реальные и общедоступные датасеты разных типов: русскоязычные e-commerce отзывы (`RuReviews`, `Perekrestok Reviews`) для sentiment analysis, классический корпус обманных отзывов `OpSpam`, графовый benchmark `FraudDataset (Yelp)` и современный мультиязычный набор `MAiDE-up` для анализа AI-generated fake reviews. Для работы с неоднородной разметкой предлагается унифицированная схема данных, поддерживающая частично размеченные записи.

Практическая значимость исследования состоит в построении воспроизводимого контура `data -> training -> export -> API -> web UI`. В архитектуре веб-системы предусмотрен не только вывод классов и вероятностей, но и прозрачный explainability-слой, позволяющий отображать распределения вероятностей по задачам и информацию об усечении входного текста. Ожидается, что мультитаск-модель обеспечит конкурентоспособное качество по sentiment classification и более устойчивое поведение в authenticity detection по сравнению с отдельными text-only baselines.
...
3. В экспериментальную схему включаются не только классические deceptive review corpora, но и современные AI-generated fake review datasets.
4. Исследование соединяет алгоритмический и системный вклад: от модели и метрик до API и explainability-oriented web interface.
...
| `FraudDataset (Yelp)` | Authenticity detection | EN | silver fraud labels | реалистичный benchmark |
| `OpSpam` | Authenticity detection | EN | truthful/deceptive | классический text-only benchmark |
| `MAiDE-up` | AI-generated fake review detection | multilingual | real vs AI-generated | современный threat-model benchmark |
| `Amazon Reviews 2023` | Auxiliary sentiment / transfer | EN | rating-derived sentiment | масштабный дополнительный источник |

Выбор именно такой комбинации объясняется тем, что в публичном поле пока отсутствует один крупный и общепризнанный датасет, который одновременно содержал бы полную e-commerce sentiment разметку и высоконадёжную authenticity annotation. Поэтому научно корректнее не маскировать эту проблему, а явно строить unified multitask corpus из нескольких репрезентативных источников.
...
- `H3`: single-task Transformer уступает мультитаск-модели в сценариях смешанного корпуса и partial supervision;
- `H4`: explainability-oriented API делает систему более пригодной для практического анализа.
...
В-третьих, explainability-слой в текущем проекте является probabilistic transparency layer, а не полноценной causal interpretability system. Он повышает прозрачность использования, но не решает полностью проблему объяснимости.

## 10. Обсуждение

С научной точки зрения наиболее интересным исходом будет не просто достижение максимального F1 на одном корпусе, а качественный анализ transfer dynamics between tasks. Если совместное обучение действительно помогает authenticity detection without harming sentiment quality, то это будет сильным аргументом в пользу мультитаск-архитектур для прикладных review analysis systems.
...
Следующий этап исследования заключается в полном прогоне экспериментов на локально подготовленных реальных корпусах, заполнении таблиц с метриками, проведении error analysis и сравнении `classical baseline -> single-task Transformer -> multitask Transformer`. После этого данный черновик может быть доведён до полноценной статьи для университетской публикации или использован как основа главы диссертации.
...
> В статье предлагается воспроизводимый мультитаск-подход к совместному определению тональности и достоверности отзывов в электронной коммерции, объединяющий трансформерную модель, унифицированный partially labeled dataset format и веб-систему с вероятностным explainability-слоем.
